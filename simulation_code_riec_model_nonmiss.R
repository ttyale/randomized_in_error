...
rie_sampler<-function(df=df, ITER=10000, beta0=3.1){
  ...
  Sigma1<-Sigma0<-Gamma0<-Gamma1<-Lambda<-diag(p)*1000
  ...
  ##store chain S
  ALPHA1<-ALPHA0<-GAMMA0<-GAMMA1<-matrix(NA,p,ITER)
  ...
  for(i in 1:ITER){
    ...
    tgt<-intersect(which(D==1),which(S==0)) 
    if(length(tgt)==1){
      v=solve(tcrossprod(X[tgt,])/tau2+solve(Gamma1))
      m=v%*%((X[tgt,])*(Y[tgt])/tau2)
    }else{
      v=solve(crossprod(X[tgt,])/tau2+solve(Gamma1))
      m=v%*%(t(X[tgt,])%*%Y[tgt]/tau2)
    }
    gamma1=rmvnorm(1,m,v)
    GAMMA1[,i]<-gamma1
    ...
    tgt2<-intersect(which(S==0),which(D==1))
    
    rate<-ff+0.5*(sum((Y[tgt1]-X[tgt1,]%*%t(gamma0))^2))+
      0.5*(sum((Y[tgt2]-X[tgt2,]%*%t(gamma1))^2))
    tau2<-rgamma(1,shape=(ee+sum((1-S),na.rm=T)/2),rate=rate)^(-1)
    TAU[i]<-tau2
  ...
  return(list(ALPHA1=ALPHA1,ALPHA0=ALPHA0, GAMMA0=GAMMA0,GAMMA1=GAMMA1,BETA=BETA,SIGMA=SIGMA, TAU=TAU, GV=GV,TE=TE))
}
...
sim_function<-function(S=10000, nsim=500, N=100, beta0=3.1){
  ...
  ALPHA1<-ALPHA0<-GAMMA0<-GAMMA1<-array(NA,c(nsim,p,S))
  ...
  output<-list(ALPHA1=ALPHA1,ALPHA0=ALPHA0, GAMMA0=GAMMA0,GAMMA1=GAMMA1,
               BETA=BETA,
               GV = GV,
               SIGMA=SIGMA,
               TAU=TAU,
...
saveRDS(sim1, "/home/cl2492/palmer_scratch/SACE/RIE/RIE_nonmiss/rie_nonmiss_simulation_n250_erate_30_sim_001-500.RDS" )

set.seed(10137341)
sim2<-sim_function(S=10000, nsim=500, N=250, beta0=1.6)
saveRDS(sim2, "/home/cl2492/palmer_scratch/SACE/RIE/RIE_nonmiss/rie_nonmiss_simulation_n250_erate_30_sim_501-1000.RDS" )

#2
#N=250; error rate = 15%
set.seed(57928354)
sim3<-sim_function(S=10000, nsim=500, N=250, beta0=3.1)
saveRDS(sim3, "/home/cl2492/palmer_scratch/SACE/RIE/RIE_nonmiss/rie_nonmiss_simulation_n250_erate_15_sim_001-500.RDS" )

set.seed(8623342)
sim4<-sim_function(S=10000, nsim=500, N=250, beta0=3.1)
saveRDS(sim4, "/home/cl2492/palmer_scratch/SACE/RIE/RIE_nonmiss/rie_nonmiss_simulation_n250_erate_15_sim_501-1000.RDS" )
...
saveRDS(sim5, "/home/cl2492/palmer_scratch/SACE/RIE/RIE_nonmiss/rie_snonmiss_imulation_n500_erate_30_sim_001-500.RDS" )

set.seed(58173643)
sim6<-sim_function(S=10000, nsim=500, N=500, beta0=1.6)
saveRDS(sim6, "/home/cl2492/palmer_scratch/SACE/RIE/RIE_nonmiss/rie_nonmiss_simulation_n500_erate_30_sim_501-1000.RDS" )

#4
#N=500; error rate = 15%
set.seed(768392234)
sim7<-sim_function(S=10000, nsim=500, N=500, beta0=3.1)
saveRDS(sim7, "/home/cl2492/palmer_scratch/SACE/RIE/RIE_nonmiss/rie_nonmiss_simulation_n500_erate_15_sim_001-500.RDS" )

set.seed(58173589)
sim8<-sim_function(S=10000, nsim=3, N=500, beta0=3.1)
saveRDS(sim8, "/home/cl2492/palmer_scratch/SACE/RIE/RIE_nonmiss/rie_nonmiss_simulation_n500_erate_15_sim_501-1000.RDS" )
